...
char* sdds::Item::GetName() const{
    return m_name;
}

int sdds::Item:: getPrice() const{
    return m_price;
}

bool sdds::Item:: getTax() const{
    return m_taxed;
}

const char* sdds::Item:: get_sku() const {
    return m_sku;
}
...
void sdds::Item::setQuantity(int q) {
    m_quantity = q;
}
...
void sdds::Item::write(ostream& os) const {
    if(ErrorState!= NULL && ErrorState->operator bool()){
        ...
        os << "=============v" << endl;
        os << "Name:        " << setw(10) << left << m_name << endl
           << "Sku:         " << setw(10) << left << m_sku << std::endl
           << "Price:       " << setw(10) << left << fixed << setprecision(2) << m_price << endl
           << "Price + tax: ";
        if (m_taxed) {
            os << setw(10) << left << fixed << setprecision(2) << cost() << endl;
        }
        else {
            os << "N/A" << endl;
        }
